app.controller('sales_ctrl', ['$scope', '$http', 'userauth',  function ($scope, $http, userauth) {
    ...
    $scope.calprice = function () {
       ...
       /* $scope.imp_precio_por_aticulos = $scope.entity.price * $scope.entity.amount;
        $scope.imp_iva = ($scope.imp_precio_por_aticulos * $scope.entity.IVArate) / 100;
        $scope.imp_precio_por_aticulos_mas_iva = $scope.imp_precio_por_aticulos + $scope.imp_iva;
        $scope.monto_descuento = ($scope.imp_precio_por_aticulos_mas_iva * $scope.entity.discount) / 100;
        $scope.importe_con_descuento = $scope.imp_precio_por_aticulos_mas_iva - $scope.monto_descuento;

        $scope.entity.IVAvalue = $scope.imp_iva;
        if (!$scope.atras) {

            $scope.entity.fullprice = isNaN($scope.importe_con_descuento) ? 0 : $scope.importe_con_descuento.toFixed(2);
        } */

        $scope.entity.price_base =  (parseFloat($scope.entity.price) * 100) / (parseFloat($scope.entity.IVArate) + 100); //$scope.entity.price - (($scope.entity.price * $scope.entity.IVArate) / 100);
    ...
    $scope.$watch('entity.price + entity.amount + entity.IVArate + entity.discount', function () {
 ...
 function validapresventa(sale) {
        ...
        if (isNaN(sale.fullprice) || sale.fullprice <= 0) {
            ...
            $http({
               ...
               // console.log(response);
                if (response.data.codigo !== 500) {
                    ...
                    $scope.entity.price = response.data.result.price + ((response.data.result.price*response.data.result.tax) / 100);
                    $scope.entity.price = $scope.entity.price.toFixed(3)
    ...
    $scope.addsale = function () {
        ...
        if (!validaventa(entidad)) {
            ...
            if( $scope.salesarray.length > 0) {
                for (var i in  $scope.sales-----  array) {
                 if (!add){ add = iguales(entidad, $scope.salesarray[i], i).pass; index = iguales(entidad, $scope.salesarray[i], i).index}
                }
                ...
                if (add) {
                    ...
                    $scope.salesarray[index].fullprice =  $scope.fullprices(($scope.salesarray[index].price*  $scope.salesarray[index].amount) + $scope.salesarray[index].IVAvalue,  $scope.salesarray[index].discount);
    ...
    $scope.findproduct = function () {
       $scope.header = false;
        if ($scope.criterio !== ''){
            ...
            $http({
                method: 'GET',
                url: '/products/find/' + $scope.criterio
            }).then(function successCallback(response) {
